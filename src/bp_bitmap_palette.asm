...
PAL2_MIN                = 0
PAL2_MAX                = 7
PAL2_RRR_MASK           = %111'000'00           ; Mask red color.
PAL2_RRR_MASKN          = %000'111'11           ; Mask all but red color.
PAL2_RRR_INC            = %001'000'00           ; Increment/decrement red color.

PAL2_GGG_MASK           = %000'111'00           ; Mask green color.
PAL2_GGG_MASKN          = %111'000'11           ; Mask all but green color.
PAL2_GGG_INC            = %000'001'00           ; Increment/decrement green color.

PAL2_BB_MASK            = %000'000'11
PAL2_BB_MASKN           = %111'111'00

PAL2_B_MASK             = %0000000'1
...
;  - DE: given color with decremented brightness.
    MACRO bp.BrightnessDown

    ; ##########################################
    ; Decrement red color (RRR'xxx'xx)
    LD A, E
    AND bp.PAL2_RRR_MASK                    ; Reset all bits but red.

    CP bp.PAL2_MIN                          ; Do not decrement if red is already at 0
...
.afterDecrementRed
    ...
    LD A, E
    AND bp.PAL2_GGG_MASK                    ; Reset all bits but green.

    CP bp.PAL2_MIN                          ; Do not decrement if green is already at 0.
...
.afterDecrementGreen
    ...
    LD A, E
    AND bp.PAL2_BB_MASK                         ; A contains 000'000'BB.
    ...
    LD A, E
    AND bp.PAL2_BB_MASKN                        ; Load RRR'GGG'BB into A and reset BB, because we will set it to new value with XOR.
    XOR B
    LD E, A
.afterDecrementBlue 
