/*
  Copyright (c) 2027 Maciej Miklas
  Licensed under the Apache License, Version 2.0. See the LICENSE file for details.
*/
;----------------------------------------------------------;
;                     Fred Movement                      ;
;----------------------------------------------------------;
    MODULE jm

; Hovering/Standing
; The counter increments with each frame when no up/down is pressed.
; When it reaches #HOVER_START_D250, Fred will start hovering.
jetInactivityCnt        DB 0

; Start times to change animations.
HOVER_START_D250        = 250
STAND_START_D30         = 30
JSTAND_START_D15        = 15

;----------------------------------------------------------;
;----------------------------------------------------------;
;                        MACROS                            ;
;----------------------------------------------------------;
;----------------------------------------------------------;

;----------------------------------------------------------;
;                     _JoyCntEnabled                       ;
;----------------------------------------------------------;
; Disable joystick and, therefore, control over the Fred.
; Return:
;  - YES: Z is reset (JP Z).
;  - NO:  Z is set (JP NZ).
    MACRO _JoyCntEnabled

    LD A, (gid.joyOffCnt)
    OR A                                        ; Same as CP 0, but faster.
    JR Z, .joyEnabled                           ; Jump if joystick is enabled -> #joyOffCnt > 0.

    ; ##########################################
    ; Joystick is disabled
    DEC A                                       ; Decrement disabled counter.
    LD (gid.joyOffCnt), A

    ; Joystick will enable on the next loop?
    OR A                                        ; Same as CP 0, but faster.
    JR NZ, .afterEnableCheck

    ; Yes, this was the last blocking loop.
    CALL gc.JoyWillEnable
.afterEnableCheck

    ; ##########################################
    ; Allow input processing if Fred is close to the platform and #joyOffCnt is > 0. It allows, for example, to move left/right when
    ; hitting the platform from below and pressing up + left (or right). 
    ; We can have the following situation: Fred is below the platform and is not bumping off anymore because it's close long enough.
    ; The player still keeps pressing up and simultaneously, let's say, left. We want to allow movement to the left, but not up.
    ; Because #joyOffCnt > 0, the function #_MainLoop000OnDisabledJoy will be executed. It will move Fred one pixel down, which is good
    ; because pressing up has moved him one pixel up. To allow movement left, we ignore #joyOffBump because it is so small that we know
    ; that Fred is right below the platform. Keeping #joyOffCnt > 0 reverses Joystick's movement up, ignoring #joyOffBump allows movement to the left.

    LD A, (pl.joyOffBump)
    CP pl.PL_BUMP_JOY_DEC_D1+1
    JR C, .joyEnabled

    _NO
    JR .end                                     ; Do not process input, as the joystick is disabled.

.joyEnabled                                     ; Process input.
    _YES

.end
    ENDM                                        ; ## END of the macro #

;----------------------------------------------------------;
;                  _MovementInactivity                     ;
;----------------------------------------------------------;
; It gets executed as a last procedure after the input has been processed, and there was no movement from joystick.
    MACRO _MovementInactivity

    ; Increment inactivity counter.
    LD A, (jetInactivityCnt)
    INC A
    LD (jetInactivityCnt), A 

    ; ##########################################
    ; Should Fred hover?
    LD A, (jt.jetAir)
    OR A                                        ; Is Fred in the air (same as CP jt.JT_STATE_INACTIVE_D0 but faster) ?
    JR Z, .afterHoover                          ; Jump if not flaying.

    LD A, (jt.jetAir)
    CP jt.AIR_HOOVER_D11                            ; Fred is in the air, but is he hovering already?
    JR Z, .afterHoover                          ; Jump if already hovering.

    ; Fred is in the air, not hovering, but is he not moving long enough?
    LD A, (jetInactivityCnt)
    CP HOVER_START_D250
    JR NZ, .afterHoover                         ; Fred is not moving, by sill not long enough to start hovering.

    ; Fred starts to hover!
    LD A, jt.AIR_HOOVER_D11
    CALL jt.SetFredStateAir

    LD A, js.SDB_HOVER
    CALL js.ChangeFredSpritePattern
    JR .end                                     ; Already hovering, do not check standing.
.afterHoover

    ; ##########################################
    ; Fred is not hovering, but should he stand?
    LD A, (jt.jetGnd)
    CP jt.JT_STATE_INACTIVE_D0                  ; Is Fred on the ground already?
    JR Z, .end                                  ; Jump if not on the ground.

    LD A, (jt.jetGnd)
    CP jt.GND_STAND_D53                         ; Fred is on the ground, but is he standing already?
    JR Z, .end                                  ; Jump if already standing.

    ; ##########################################
    ; Fred is on the ground and does not move, but is he not moving long enough?
    LD A, (jetInactivityCnt)
    CP STAND_START_D30
    JR NZ, .afterStand                          ; Jump if Fred stands for too short to trigger standing.
    
    ; Transition from walking to standing.
    LD A, jt.GND_STAND_D53
    CALL jt.SetFredStateGnd

    LD A, js.SDB_STAND                          ; Change animation.
    CALL js.ChangeFredSpritePattern
    JR .end
.afterStand

    ; We are here because: jetInactivityCnt > 0 and jetInactivityCnt < STAND_START_D30 
    ; Fred stands still for a short time, not long enough, to play standing animation, but at least we should stop walking animation.
    LD A, (jt.jetGnd)
    CP jt.GND_WALK_D51
    JR NZ, .end                                 ; Jump if not walking.
    
    CP jt.GND_JSTAND_D52
    JR Z, .end                                  ; Jump already j-standing (just standing - for a short time).

    LD A, (jetInactivityCnt)
    CP JSTAND_START_D15
    JR NC, .end                                 ; Jump if Fred stands for too short to trigger j-standing.

    ; Stop walking immediately and stand still.
    LD A, jt.GND_JSTAND_D52
    CALL jt.SetFredStateGnd

    LD A, js.SDB_JSTAND                         ; Change animation.
    CALL js.ChangeFredSpritePattern

.end
    ENDM                                        ; ## END of the macro #

;----------------------------------------------------------;
;----------------------------------------------------------;
;                   PUBLIC FUNCTIONS                       ;
;----------------------------------------------------------;
;----------------------------------------------------------;


;----------------------------------------------------------;
;                      #JoyMoveUp                          ;
;----------------------------------------------------------;
JoyMoveUp

    CALL _CanFredMove
    RET NZ                                      ; Do not process input on disabled joystick.

    ; ##########################################
    ; Direction change: down -> up.
    LD A, (gid.jetDirection)
    AND gid.MOVE_UP_MASK                        ; Are we moving Up already?
    CP gid.MOVE_UP_MASK
    JR Z, .afterDirectionChange

    ; We have direction change!
    LD A, (gid.jetDirection)                    ; Update #jetState by resetting down and setting up.
    RES gid.MOVE_DOWN_BIT_D3, A
    SET gid.MOVE_UP_BIT_D2, A
    LD (gid.jetDirection), A
.afterDirectionChange

    ; ##########################################
    CALL _SholdProcessJoyOnOverheat
    RET NZ

    ; ##########################################
    CALL _JoystickMoves

    ; ##########################################
    ; Decrement Y position.
    LD A, (jpo.jetY)    
    CP _GSC_Y_MIN_D15                           ; Do not decrement if Fred has reached the top of the screen.
    JR Z, .afterDec
    CALL jpo.DecFredY
.afterDec

    ; ##########################################
    CALL gc.FredPlatformTakesOff                ; Transition from walking to flaying.

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                      JoyMoveRight                        ;
;----------------------------------------------------------;
JoyMoveRight

    CALL _CanFredMove
    RET NZ                                      ; Do not process input on disabled joystick.

    ; ##########################################
    ; Direction change: left -> right
    LD A, (gid.jetDirection)
    AND gid.MOVE_RIGHT_MASK                     ; Are we moving right already?
    CP gid.MOVE_RIGHT_MASK
    JR Z, .afterDirectionChange

    ; We have direction change!
    LD A, (gid.jetDirection)                    ; Reset left and set right.
    RES gid.MOVE_LEFT_BIT_D0, A
    SET gid.MOVE_RIGHT_BIT_D1, A
    LD (gid.jetDirection), A
.afterDirectionChange

    ; ##########################################
    CALL _SholdProcessJoyOnOverheat
    RET NZ

    ; ##########################################
    CALL _JoystickMoves
    CALL _StandToWalk
    CALL jpo.IncFredX

    ; ##########################################
    ; Fall from the platform?
    CALL pl.FredFallingFromPlatform

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                       JoyMoveLeft                        ;
;----------------------------------------------------------;
JoyMoveLeft

    CALL _CanFredMove
    RET NZ                                      ; Do not process input on disabled joystick.

    ; ##########################################
    ; Direction change: right -> left
    LD A, (gid.jetDirection)
    AND gid.MOVE_LEFT_MASK                      ; Are we moving left already?
    CP gid.MOVE_LEFT_MASK
    JR Z, .afterDirectionChange                 ; Fred is moving left already -> end.

    ; We have direction change! 
    LD A, (gid.jetDirection)                    ; Reset right and set left.
    RES gid.MOVE_RIGHT_BIT_D1, A
    SET gid.MOVE_LEFT_BIT_D0, A
    LD (gid.jetDirection), A
.afterDirectionChange

    ; ##########################################
    CALL _SholdProcessJoyOnOverheat
    RET NZ

    ; ##########################################
    CALL _JoystickMoves 
    CALL _StandToWalk
    CALL jpo.DecFredX

    ; ##########################################
    ; Fall from the platform?
    CALL pl.FredFallingFromPlatform

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                       JoyMoveDown                        ;
;----------------------------------------------------------;
JoyMoveDown

    CALL _CanFredMove
    RET NZ                                      ; Do not process input on disabled joystick.

    ; ##########################################
    ; Cannot move down when walking
    LD A, (jt.jetGnd)
    OR A                                        ; Same as: CP jt.JT_STATE_INACTIVE_D0
    RET NZ

    ; ##########################################
    ; Direction change? 
    LD A, (gid.jetDirection)
    AND gid.MOVE_DOWN_MASK                      ; Are we moving down already?
    CP gid.MOVE_DOWN_MASK
    JR Z, .afterDirectionChange

    ; We have direction change!
    LD A, (gid.jetDirection)                    ; Update #jetState by resetting Up/Hover and setting down.
    RES gid.MOVE_UP_BIT_D2, A
    SET gid.MOVE_DOWN_BIT_D3, A
    LD (gid.jetDirection), A
    
    CALL js.ChangeFredSpriteOnFlyDown
.afterDirectionChange

    ; ##########################################
    CALL _SholdProcessJoyOnOverheat
    RET NZ

    ; ##########################################
    CALL _JoystickMoves

    ; ##########################################
    ; Increment Y position
    LD A, (jpo.jetY)
    CP _GSC_JET_GND_D217                        ; Do not increment if Fred has reached the ground.
    JR Z, .afterInc
    CALL jpo.IncFredY                            ; Move Fred 1px down.
.afterInc   

    ; ##########################################
    ; Landing on the ground
    LD A, (jpo.jetY)
    CP _GSC_JET_GND_D217
    CALL Z, pl.FredLanding                      ; Execute landing on the ground if Fred has reached the ground.

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                  JoyMoveDownRelease                      ;
;----------------------------------------------------------;
JoyMoveDownRelease

    CALL js.ChangeFredSpriteOnFlyUp

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                JoystickMoveProcessed                    ;
;----------------------------------------------------------;
; It gets executed as a last procedure after the input has been processed, regardless of whether there was movement, or not.
JoystickMoveProcessed

    CALL _CanFredMove
    RET NZ                                      ; Do not process input on disabled joystick.

    ; ##########################################
    ; Ignore the situation when Fred stands on the ground and only down is present. This does not count as movement.
    LD A, (jt.jetGnd)
    CP jt.JT_STATE_INACTIVE_D0
    JR Z, .afterDownOnGround

    ; Fred is on the ground, but is only down key pressed (without left/right)?
    LD A, (gid.joyDirection)
    CP gid.MOVE_DOWN_MASK
    JR Z, .inactive                             ; Jump if, Fred is on the ground and only down is pressed, we have inactivity, skip other checks.

.afterDownOnGround
    
    ; ##########################################
    ; Is there a movement?
    LD A, (gid.joyDirection)
    OR A                                        ; Same as: CP gid.MOVE_INACTIVE_D0
    RET NZ                                      ; Jump if there is a movement.

.inactive

     _MovementInactivity

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;----------------------------------------------------------;
;                   PRIVATE FUNCTIONS                      ;
;----------------------------------------------------------;
;----------------------------------------------------------;

;----------------------------------------------------------;
;               _SholdProcessJoyOnOverheat                 ;
;----------------------------------------------------------;
; Slow down joystick input and, therefore, the speed of Fred's movement when jetpack has overheated.
; Return:
;  - YES: Process joystick input, Z is reset (JP Z).
;  - NO:  Disable joystick input processing for this loop, Z is set (JP NZ).
_SholdProcessJoyOnOverheat
    LD A, (jt.jetState)
    CP jt.FREDST_OVERHEAT_D104
    JR NZ, .yes

    LD A, (jt.jetGnd)
    CP jt.JT_STATE_INACTIVE_D0
    JR NZ, .yes

    LD A, (mld.counter000FliFLop)
    CP _GC_FLIP_ON_D1
    JR Z, .delayReached

    ; Return because #joyDelayCnt !=  #pl.PL_JOY_DELAY
    _NO
    RET
.delayReached                                   ; Delay counter has been reached.

    XOR A                                       ; Set A to 0.
    LD (gid.joyOverheatDelayCnt), A             ; Reset delay counter.

    ; Process input, because counter has been reached.

.yes
    _YES

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                     _JoystickMoves                       ;
;----------------------------------------------------------;
; Method gets called on any joystick movement (only real key press), but not fire pressed.
_JoystickMoves

    CALL pl.ResetJoyOffBump

    dbs.SetupRocketBank
    CALL roa.UpdateRocketOnFredMove
    
    CALL pl.FredPlatformHitOnJoyMove

    ; ##########################################
    ; Reset inactivity counter as we have movement.
    XOR A                                       ; Set A to 0
    LD (jetInactivityCnt), A

    ; ##########################################
    ; Transition from hovering to flying?
    LD A, (jt.jetAir)
    CP jt.AIR_HOOVER_D11                        ; Is Fred hovering?
    JR NZ, .afterHovering                       ; Jump if not hovering.

    ; Fred is hovering, but we have movement, so switch state to fly.
    LD A, jt.AIR_FLY_D10
    CALL jt.SetFredStateAir
    
    ; Switch to flaying animation
    LD A, js.SDB_FLY
    CALL js.ChangeFredSpritePattern
.afterHovering  

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                     _StandToWalk                         ; 
;----------------------------------------------------------;
; Transition from standing/landing on ground to walking
_StandToWalk

    LD A, (jt.jetGnd)
    OR A                                        ; Same as: CP jt.JT_STATE_INACTIVE_D0
    RET Z                                       ; Exit if Fred is not on the ground.

    ; Fred is on the ground, is he already walking?
    LD A, (jt.jetGnd)   
    CP jt.GND_WALK_D51
    RET Z                                       ; Exit if Fred is already walking.

    ; Fred is standing and starts walking now.
    LD A, jt.GND_WALK_D51
    CALL jt.SetFredStateGnd
    
    LD A, js.SDB_WALK_ST
    CALL js.ChangeFredSpritePattern

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                      _CanFredMove                         ;
;----------------------------------------------------------;
; Return:
;  - YES: Z is reset (JP Z).
;  - NO:  Z is set (JP NZ).
_CanFredMove

    _JoyCntEnabled
    RET NZ

    ; ##########################################
    ; Joystick disabled if Fred is inactive.
    LD A, (jt.jetState)
    OR A                                        ; Sama as: CP jt.JT_STATE_INACTIVE_D0
    JR NZ, .jetActive

    ; Do not process input.
    _NO
    RET
.jetActive

    ; ##########################################
    LD A, (jt.jetState)
    CP jt.FREDST_RIP_D103
    JR NZ, .afterRip                            ; Do not process input if Fred is dying.

    ; Do not process input, Fred is dying.
    _NO
    RET
.afterRip

    ; ##########################################
    ; Process input
    _YES

    RET                                         ; ## END of the function ##

;----------------------------------------------------------;
;                       ENDMODULE                          ;
;----------------------------------------------------------;
    ENDMODULE